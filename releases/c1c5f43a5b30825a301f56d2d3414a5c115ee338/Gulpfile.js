...
gulp.task('default',function() {
    const text  = `\n\n${$.gutil.colors.inverse('        COMMANDS        ')}`
        ...
        + `\n${$.gutil.colors.bold('└─ Performs all tasks, including Critical CSS and processing HTML files.\n\n\`gulp release\` advances the version number in \`package.json\` by 0.0.1. Running \`gulp release:feature\` will advance it by 0.1.0; and running \`gulp release:major\` will advance it by 1.0.0.')}\n`
...
gulp.task('release:tasks', ['css:cleaned', 'img:cleaned', 'ejs:release', 'webpack'], function() {
    return notifier.notify({ 'title': name, 'icon': notifyIconPath, 'message': 'Release Complete' });
});

// [gulp commit]
gulp.task('deploy:clean', ['release', 'cleanCritCss'], function() {
    return gulp.src([paths.distCss, paths.distImg, paths.distJs, bases.html + '**/*.twig'], {read: false})
...
gulp.task('deploy', ['deploy:clean'], function() {
    ...
    if (commitMessage) {
        ...
        if (publishAnswers.commitRelease && publishAnswers.message) {
            exec.spawnSync(`git add -A && git commit -m "${ publishAnswers.message }" && git push && git status`, [], { stdio: 'inherit', shell: true });
        }
    } else {
...
function bumpVersionNumber(level) {
    const version = runBump ? semver.inc(vars.version, level) : vars.version;
...
function cssHandler() {
    return gulp.src(paths.filesScss)
        ...
        .pipe($.sassGlob())
        .pipe(sassVariables({ $release: release ? 'production' : 'development' }))
...
gulp.task('ejs:release', ['favicons'], function() {
...
gulp.task('favicons', ['favicons:generate'], function() {
    return runFavicon ? gulp.src(bases.build + 'favicon/**/*')
        .pipe(gulp.dest(paths.distImg + 'meta/')) : true;
});
gulp.task('favicons:generate', ['cleanFavicon'], function() {
    return runFavicon ? gulp.src(notifyIconPath)
        .pipe($.favicons({
            html: bases.build + 'favicon/favicons.html',
            path: bases.site + 'img/meta',
        }))
        .pipe(gulp.dest(bases.build + 'favicon')) : true;
});
