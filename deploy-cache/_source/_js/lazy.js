...
function testFunction(el, rotation = '15') {
  const currentDegrees = el.hasAttribute('data-rotation-degrees')
    ? parseFloat(el.getAttribute('data-rotation-degrees'))
    : '0';
  ...
  if (element.getAttribute('data-lazy-animate') !== '' || element.getAttribute('data-lazy-animate-exit') !== '') {
    ...
    switch (viewportEvent) {
      case 'enter':
        ...
        args = element.hasAttribute('data-lazy-animate-args')
          ? JSON.parse(element.getAttribute('data-lazy-animate-args'))
          : null;
        break;
      case 'exit':
        func = element.getAttribute('data-lazy-animate-exit');
        args = element.hasAttribute('data-lazy-animate-args-exit')
          ? JSON.parse(element.getAttribute('data-lazy-animate-args-exit'))
          : null;
        break;
    }
      ...
      for (let i = 0, l = bgData.css.length; i < l; i++) {
        ...
        css += `${
          media
            ? `@media ${
                retina
                  ? `${mq != '' ? `${mq} and ` : ''}(-webkit-min-device-pixel-ratio: 2), ${
                      mq != '' ? `${mq} and ` : ''
                    }(min-resolution: 192dpi)`
                  : mq
              } { `
            : ''
        }.${elementClass} { background-image: url('${bgData.css[i].url}');${extra} }${media ? ' }' : ''}`;
      }
...
export default function(lazyConfig = {}) {
  ...
  for (let i = 0, l = lazyLoadElements.length; i < l; i++) {
    const elementOffset = lazyLoadElements[i].hasAttribute('data-lazy-load-offset')
      ? lazyLoadElements[i].getAttribute('data-lazy-load-offset')
      : 500;
  ...
  for (let i = 0, l = lazyAnimateElements.length; i < l; i++) {
    const elementOffset = lazyAnimateElements[i].hasAttribute('data-lazy-animate-offset')
      ? parseFloat(lazyAnimateElements[i].getAttribute('data-lazy-animate-offset'))
      : -200;
    const elementDelay = lazyAnimateElements[i].hasAttribute('data-lazy-animate-delay')
      ? parseFloat(lazyAnimateElements[i].getAttribute('data-lazy-animate-delay'))
      : 0;
