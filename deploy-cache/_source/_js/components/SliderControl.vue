<template>
  <div class="c_slider_control">
    <div
      class="c_slider_control__back"
      v-html="labelBack"
      v-if="showBack"
      @click="setSlideIndex(currentSlide - 1)"
    ></div>
    <div
      class="c_slider_control__indicators"
      v-if="showIndicators && indicatorType === 'dots' && maxNodes >= totalSlides"
    >
      <div
        class="c_slider_control__indicator_dot"
    ...
    <div
      class="c_slider_control__indicators"
      v-else-if="showIndicators && indicatorType === 'numbers' && maxNodes >= totalSlides"
    >
      <span class="c_slider_control__indicator_number">{{ currentSlide + 1 }}</span
      >{{ numberIndicatorDivider }}<span class="c_slider_control__indicator_number">{{ totalSlides }}</span>
    </div>
    <div
      class="c_slider_control__next"
      v-html="labelNext"
      v-if="showNext"
      @click="setSlideIndex(currentSlide + 1)"
    ></div>
